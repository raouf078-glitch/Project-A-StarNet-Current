...
import { Loader as Loader2, DollarSign, Check, X, Clock, Image, ExternalLink } from 'lucide-react'
...
export default function AdminDeposits() {
  ...
  const load = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('deposits')
        .select(`*, account:deposit_accounts(id, account_name, account_number, bank_name)`)
  ...
  const process = async (row, status, notes = '') => {
    if (busy) return
    setBusy(true)
    try {
      ...
      if (error) throw error
      flash(status === 'approved' ? 'تم اعتماد الإيداع ✓' : 'تم رفض الإيداع')
      load()
    } catch (e) { flash(e.message || 'تعذّر المعالجة') }
    finally { setBusy(false) }
  }

  const STATUS = {
    pending: { label: 'قيد المراجعة', color: 'bg-amber-50 text-amber-600', icon: Clock },
    approved: { label: 'معتمد', color: 'bg-emerald-50 text-emerald-600', icon: Check },
    rejected: { label: 'مرفوض', color: 'bg-rose-50 text-rose-500', icon: X },
  }
    ...
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-10 text-center">
          <DollarSign size={26} className="text-gray-300 mx-auto mb-2" />
              ...
              <div key={row.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-3.5 space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-9 h-9 rounded-lg bg-amber-50 flex items-center justify-center"><DollarSign size={17} className="text-amber-600" /></div>
                    <div>
                      <p className="font-black text-gray-800 text-sm">{Number(row.amount).toLocaleString()} ر.ي</p>
                      <p className="text-[10px] text-gray-400">{new Date(row.created_at).toLocaleString('ar')}</p>
                    </div>
                  </div>
                ...
                <div className="text-xs text-gray-500 space-y-0.5 bg-gray-50 rounded-lg p-2.5">
                  {row.sender_name && <p>المرسل: <span className="font-bold text-gray-700">{row.sender_name}</span></p>}
                  {row.transfer_number && <p>رقم الحوالة: <span className="font-bold text-gray-700" dir="ltr">{row.transfer_number}</span></p>}
                  {row.account && <p>الحساب: <span className="font-bold text-gray-700">{row.account.account_name} — {row.account.bank_name}</span></p>}
                  {row.notes && <p>ملاحظات: <span className="text-gray-600">{row.notes}</span></p>}
                </div>

                {row.proof_url && (
                  <a href={row.proof_url} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 text-xs font-bold text-blue-600 bg-blue-50 rounded-lg px-3 py-2 w-fit">
                    <Image size={14} /> عرض الإيصال <ExternalLink size={12} />
                  </a>
                )}

                {row.status === 'pending' && (
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => process(row, 'approved')} disabled={busy}
                    ...
                    <button onClick={() => { const n = prompt('سبب الرفض (اختياري):') || ''; process(row, 'rejected', n) }} disabled={busy}
